...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // const firstName=await axios.get(`http://localhost:5000/api/user/users{firstName}`);
      console.log(response.data);
      
      // Assuming the server sends a 'message' property on success
      if (response.data.message === 'Login successful') {
        ...
        router.push(`/Shorten?firstName=${encodeURIComponent(firstName)}`);
          ...
          <div className='flex z-20 items-center justify-center'>
            <div className='mt-12 font-fam p-8 rounded-xl shadow-2xl'>
              ...
              <div className='mb-4'>
                ...
                >
                  <svg
                    className='mr-3 h-5 inline pb-0.5'
                ...
                {showEmailForm && (
                  <form className='space-y-4 mt-10 mb-10' onSubmit={handleSubmit}>
                    ...
                    {loginError && (
                      <p className='text-red-500 flex justify-center'>{loginError}</p>
                    )}
                    {successMessage && (
                      <p className='text-green-500 flex justify-center'>{successMessage}</p>
                    )}
                ...
                <p className='text-lg text-bold text-center mt-6'>
                  ...
                  <Link href='./register'>
                    <span
                      style={{ color: '#144ee3', textDecoration: 'underline' }}
                      className='ml-2'
                    >
                      Sign up
                    </span>
